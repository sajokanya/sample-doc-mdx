...
Cirrus Sync uses multithreading to improve performance and efficiency. This technique splits tasks into multiple threads that can be executed concurrently.

The sync client uses multiple threads to:
- Scan local files for changes or deletions
- Upload and download updates  
- Compute file differences

**Sync Types:**
- `bidirectional` - Synchronizes changes in both directions
- `upload_only` - Only uploads local changes to remote
- `download_only` - Only downloads remote changes to local

**Conflict Resolution:**
- `manual` - Requires user intervention to resolve conflicts
- `latest_wins` - Automatically uses the most recently modified version
